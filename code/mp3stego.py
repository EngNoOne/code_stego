...
def mp3stego(message,password,carrier,outfilename):
    subprocess.Popen(["E:\lectures\semister 7\steganography\project\MP3Stego-main\MP3Stego-main\MP3Stego\Encode.exe"
                      ,"-E"
                      ,message
                      ,"-P"
                      ,password
...
def mp3stego_extract(password, carrier):
    try:
        mp3stego_path = r"E:\lectures\semister 7\steganography\project\MP3Stego-main\MP3Stego-main\MP3Stego"

        # Ensure the tables directory exists
        tables_path = os.path.join(mp3stego_path, "tables")
        if not os.path.exists(tables_path):
            messagebox.showerror("Error", f"Tables directory missing: {tables_path}")
            return

        # Run the Decode.exe command in the correct directory
        result = subprocess.run(
            [
                os.path.join(mp3stego_path, "Decode.exe"),
                "-X",  # Extraction flag
                "-P", password,
                carrier
            ],
            cwd=mp3stego_path,  # Ensure working directory is set correctly
            capture_output=True,
            text=True
        )
        ...
        if result.returncode != 0:
            messagebox.showerror("Error", f"Extraction failed: {result.stderr.strip()}")
        else:
            messagebox.showinfo("Success", "Hidden message extracted successfully!")
            print(result.stdout)  # Debugging/logging output
